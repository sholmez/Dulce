...
$query = "SELECT * FROM dishes WHERE type = 'main_dish'";
$stmt = $conn->prepare($query);
$stmt->execute();
$main_dishes = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
        ...
        <div class="container">
          <div class="breadcrumbs-custom__inner">
            <ul class="breadcrumbs-custom__path">
              ...
              <li class="active">Основні страви</li>
            </ul>
          </div>
        </div>
      </section>
      <section class="text-center">
        <section class="section parallax-container" data-parallax-img="images/parallax-07.jpg">
        ...
        <div class="container">
            <div class="row row-80">
                <?php if (!empty($main_dishes)): ?>
                    <?php foreach ($main_dishes as $main_dish): ?>
                        <div class="col-md-6 col-lg-4">
                            <!-- Thumb corporate-->
                            <div class="thumb thumb-corporate">
                                <div class="thumb-corporate__main">
                                    <img src="images/<?php echo $main_dish['image']; ?>" alt="<?php echo $main_dish['name']; ?>" width="418" height="315"/>
                                </div>
                                <div class="thumb-corporate__caption">
                                    <p class="thumb__title">
                                        <a href="single-product.php?id=<?php echo $main_dish['id']; ?>"><?php echo $main_dish['name']; ?></a>
                                    </p>
                                    <p class="thumb__subtitle">₴<?php echo $main_dish['price']; ?></p>
                                    <p><?php echo $main_dish['composition']; ?></p>
                                    <p><?php echo $main_dish['size']; ?>гр</p>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p>Нажаль наразі немає блюд для показу.</p>
                <?php endif; ?>
            </div>
        </div>
    </section>
